...
List<String> _resolveLetterhead(List<String>? letterheadLines) =>
    letterheadLines ?? buildInvoiceLetterhead(null);

Iterable<String> _invoiceTextLines(
  Farmer farmer, {
  required String currency,
  List<String>? letterheadLines,
}) sync* {
  final dateStr = DateFormat('yyyy-MM-dd').format(farmer.dateOfPurchase);
  for (final line in _resolveLetterhead(letterheadLines)) {
    yield line;
  }
  yield '';
...
Uint8List _encodeInvoiceDocx(Farmer farmer, {List<String>? letterheadLines}) {
  final body = StringBuffer()
    ...
    ..write('<w:body>');
  for (final line in _invoiceTextLines(
    farmer,
    currency: kInvoiceCurrencyPrefix,
...
pw.Widget _pdfInvoiceLetterhead(List<String> lines) {
...
pw.Document _buildFarmerInvoicePdfDoc(Farmer farmer, {List<String>? letterheadLines}) {
  const currency = kInvoiceCurrencyPrefix;
  final head = _resolveLetterhead(letterheadLines);
...
Future<void> shareFarmerInvoicePdf(
  Farmer farmer, {
  AppOrganizationSettings? organizationSettings,
}) async {
  final letterhead = buildInvoiceLetterhead(organizationSettings);
  final pdf = _buildFarmerInvoicePdfDoc(farmer, letterheadLines: letterhead);
...
Future<void> shareFarmerInvoiceDocx(
  Farmer farmer, {
  AppOrganizationSettings? organizationSettings,
}) async {
  final letterhead = buildInvoiceLetterhead(organizationSettings);
  final bytes = _encodeInvoiceDocx(farmer, letterheadLines: letterhead);

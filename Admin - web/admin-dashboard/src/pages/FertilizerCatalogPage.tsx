...
export function FertilizerCatalogPage() {
  const { items: remoteItems, loading, error } = useFertilizerCatalog();
  ...
  function addItem(e: FormEvent) {
    ...
    if (!name) {
      setFormError("Enter a fertilizer name.");
      return;
    }
    ...
    const row: FertilizerCatalogItem = {
  ...
  function updateRow(id: string, patch: Partial<FertilizerCatalogItem>) {
    setItems((prev) =>
      prev
  ...
  async function persist() {
    ...
    setSaving(true);
    try {
      await saveFertilizerCatalog(items);
      setDirty(false);
    } catch (err) {
      ...
      <div style={page}>
        ...
        <section style={card}>
          ...
          {loading && items.length === 0 && !dirty ? (
            ...
            <p style={muted}>
              No custom items yet — farmer screens use built-in presets until you add rows and save.
            </p>
          ) : (
...
function CatalogEditRow({
  ...
  row: FertilizerCatalogItem;
  onChange: (id: string, patch: Partial<FertilizerCatalogItem>) => void;

...
function allFarmerLines(f: Farmer): FertilizerType[] {
  return [
    ...omitZeroAmountLines(f.fertilizers),
    ...omitZeroAmountLines(f.pesticides),
    ...omitZeroAmountLines(f.seeds),
    ...omitZeroAmountLines(f.cscProducts),
  ];
}

/** Quantity a farmer bought per product name (summed if repeated across categories). */
function farmerQuantitiesByProduct(f: Farmer): Map<string, number> {
  const map = new Map<string, number>();
  for (const line of allFarmerLines(f)) {
...
/** Distinct product names across all farmers, in first-seen order. */
function collectProductNames(farmers: Farmer[]): string[] {
  const names: string[] = [];
...
function farmerExcelBaseColumns(f: Farmer): (string | number)[] {
  return [
    f.slNo,
...
/**
 * One row per farmer. Each distinct product (Saathi, Pan Jamuna, DAP, …) gets its own
 * column holding the quantity that farmer bought; empty when not purchased.
 */
export function downloadFarmersListExcel(farmers: Farmer[]): void {
  const productNames = collectProductNames(farmers);
  const headers = [
    ...EXCEL_BASE_HEADERS,
    ...productNames,
    "Remarks",
  ...
  const rows: (string | number)[][] = farmers.map((f) => {
    const quantities = farmerQuantitiesByProduct(f);
    const productCells = productNames.map((name) => quantities.get(name) ?? "");
    return [
      ...farmerExcelBaseColumns(f),
      ...productCells,
      dash(f.remarks),
      totalPrice(f),
    ];
